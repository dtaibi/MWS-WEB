 ...
 */
@ManagedBean(name="comparesearches")
@SessionScoped
public class CompareSearches {
	private List<Ricerca> searches;
	private String resultId="0";
	@ManagedProperty(value="#{user}")
	private User user;
	private String searchsel;
	// private List<Ricerca> userSearches;
	...
	public void openSearch(){
		RicercaDao ricdao=new RicercaDao();
		Ricerca ric= ricdao.getRicercaById(Long.parseLong(this.searchsel));
		this.searches.add(ric);
		
		TrackDao td=new TrackDao();
		Track track=new Track();
		Calendar c = new GregorianCalendar();
		track.setDate(c.getTime());
		track.setOperation("open_search");
		track.setParam1(ric.getNome());
    ...
    public List<Ricerca> getUserSearches() {
    	System.out.println("usersearches");
    	System.out.println("SIZE:_"+this.user.getUtente().getRicerca().size()+"_");
        return this.user.getUtente().getRicerca();
    }

    public List<Ricerca> getSearches() {
        return this.searches;
    }
    
    public void setSearches(List<Ricerca> searches) {
    	this.searches = searches;
    }
    public String getSearchDataPie(Ricerca ric, String nome){
    	JSONObject dpList;
    	String value=""; 
		try {
			dpList = new JSONObject(ric.getSearchDataPies());
			value =dpList.get(nome).toString();
		} catch (JSONException e) {
			e.printStackTrace();
		}
		return value.replace("chartpie2", "chartpie2_"+ric.getId()).replace("chartpie1", "chartpie1_"+ric.getId()).replace("chartpie3", "chartpie3_"+ric.getId());
    }
    
    public List<String> getSiteAvailablelist(Ricerca ric, String nome){
    	JSONObject avList;
    	System.out.println(ric);
    	List<String> sites=null; 
		try {
			avList = new JSONObject(ric.getSiteAvailablelists());
			JSONArray arr1=(JSONArray)avList.get(nome);
			sites = new ArrayList<String>();
			for (int i=0;i<arr1.length();i++)
				sites.add(arr1.get(i).toString());
		} catch (JSONException e) {
			e.printStackTrace();
		}
    	return sites;
    ...
    public List<String> getSiteSelectedlist(Ricerca ric, String nome){
    	JSONObject avList;
    	System.out.println(ric);
    	List<String> sites=null; 
		try {
			avList = new JSONObject(ric.getSiteSelectedlists());
			JSONArray arr1=(JSONArray)avList.get(nome);
			sites = new ArrayList<String>();
			for (int i=0;i<arr1.length();i++)
				sites.add(arr1.get(i).toString());
	...
	@PostConstruct
    public void init() {
		searches = new ArrayList<Ricerca>(); 
