...
public class StoryBoard {
	...
	public void saveComment(StoryboardItem sbi){
		...
		track.setParam1(storyboard.getNome());
		track.setUtente(this.user.getUtente());
		td.addTrack(track);
	}
	...
	public void getSlides(){
		...
		if (!FacesContext.getCurrentInstance().isPostback()){
				...
				StoryboardDao rdao=new StoryboardDao();
				storyboard=rdao.getStoryboardById(this.sbId);
				for (StoryboardItem sbi : storyboard.getStoryboardItem()){
					if (sbi.getType().compareTo("video")!=0)
						sbi.setWebcontent("<iframe sandbox=\"allow-forms allow-scripts\" width=\"90%\" height=\"90%\" src=\""+sbi.getContent()+"\" ></iframe>");
					else
						sbi.setWebcontent("<iframe width=\"0\" height=\"0\"></iframe><iframe width=\"90%\" height=\"90%\" src=\""+sbi.getContent().replace("http://www.youtube.com/watch?v=", "http://www.youtube.com/embed/")+"\" ></iframe>");
				}
				if (this.user!=null && this.user.getUtente()!=null){
					TrackDao td=new TrackDao();
					Track track=new Track();
					Calendar c = new GregorianCalendar();
					track.setDate(c.getTime());
					track.setOperation("load_storyboard");
					track.setParam1(storyboard.getNome());
					track.setUtente(this.user.getUtente());
					td.addTrack(track);
				}else{
					System.out.println("GET_SLIDES: user null");
				}
				System.out.println("GET_SLIDES_POSTBACK_2:"+storyboard);
				/* if (this.sbId==null){
	...
	public void saveStoryboard(){
		...
		track.setParam1(this.storyboard.getNome());
		track.setUtente(this.user.getUtente());
		td.addTrack(track);
	}
	...
	public void RemoveWebRes(StoryboardItem sr) {
		...
		storyboard.getStoryboardItem().remove(sr);
	...
	public void reorderList() {
		...
		for (String s : pos) {
			...
			StoryboardItem sbi=storyboard.getStoryboardItem().get(id);
		...
		storyboard.getStoryboardItem().clear();
		storyboard.setStoryboardItem(newstoryboardlist);
		storyboard.setUtente(this.user.getUtente());
		storyboard.setData((new GregorianCalendar()).getTime());
		StoryboardDao rd=new StoryboardDao();
		rd.addStoryboard(storyboard);
