...
@ManagedBean(name="archivesearchstudent")
@ViewScoped
public class ArchiveSearchStudent implements Serializable {
    ...
    public void init() {
		...
		/*try {
			sdate=new SimpleDateFormat( "yyyyMMdd" ).parse( "20111203" );
			edate=new SimpleDateFormat( "yyyyMMdd" ).parse( "20121009" );
	...
	public void seturlcontent(String sceneid){
		
		scenepopup=urlcontent.get(sceneid).replace("\n", "<br />");
		
	}
	
Map<Integer, ArrayList<SearchWebResult>> resultlist=new HashMap<Integer, ArrayList<SearchWebResult>>();



public Map<Integer, ArrayList<SearchWebResult>> getResultlist() {
	return resultlist;
}
...
public String searcAll(int nuovo) throws IOException{
		/*
		 try {
			    ...
			    Date result =  df.parse(this.startdate);
			    SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
			    this.startdate = formatter.format(result);
			    result =  df.parse(this.enddate);
			    this.enddate = formatter.format(result);
		} catch (ParseException e) {
			
			e.printStackTrace();
		}
		 */
		resultlist=new HashMap<Integer, ArrayList<SearchWebResult>>();
		resultlist.put(1, (new ArrayList<SearchWebResult>()));
		...
		ESearchPost eSearch;
		// eSearch= new ESearchPost("http://130.75.152.117/esearch/asimijaz_arch2950_anno/_search?pretty");
		// eSearch= new ESearchPost("http://130.75.152.117/esearch/asimijaz_arch2950_annt/_search?pretty&search_type=dfs_query_then_fetch");
		eSearch= new ESearchPost("http://130.75.152.117/esearch/asimijaz_arch2950_nestedwt/_search?pretty&search_type=dfs_query_then_fetch");
								  
		// int noOfResults=100;
		
		ArrayList<ESearchDataObject> objList;		
		// objList=(ArrayList<ESearchDataObject>) eSearch.searhByContent(this.getSearchquery(), noOfResults, this.startdate, this.enddate);
		objList=(ArrayList<ESearchDataObject>) eSearch.searhByContent(this.getSearchquery());
		...
		for (ESearchDataObject anr : objList){
				...
				r.setScene(anr.getContent());
				String shortdescr=anr.getContent().replaceAll("[^\\u0000-\\uFFFF]", "");
				if (anr.getContent().length()>=100)
					shortdescr=anr.getContent().substring(0,100).replaceAll("[^\\u0000-\\uFFFF]", "");
				// r.setDescription(tmgr.MultipleTextToCheck(this.searchterms, anr.getContent().substring(0,100)+"...", 0)+occurrences);
				r.setDescription(shortdescr+"..."+"<br />"+prettufyTstamp(anr.getTstamp()));
				teasers.add(shortdescr+"...");
				r.setSceneid(anr.getScene());
				String linkwebarchive="<a target=\"_blank\" href=\"https://web.archive.org/web/"+anr.getTstamp()+"/"+anr.getUrl()+"\">Link Web Archive</a><br />";
				String linkweb="<a target=\"_blank\" href=\""+anr.getUrl()+"\">Current Link</a><br />";
				urlcontent.put(anr.getScene(), linkwebarchive+linkweb+anr.getContent());
				r.setUrl(anr.getUrl());
				...
				/*************/
				if (resultlist.get(conta).size()==0){
					resultlist.get(conta).add(r);
				}else{
				
					int found=0;
					for (Integer key : resultlist.keySet()){
						for (SearchWebResult se2 : resultlist.get(key)){
							if (this.jaccardSimilarity(se2.getScene(), r.getScene())){
								found=key;
								break;
					...
					if (found!=0){
						resultlist.get(found).add(r);
					}else{
						ArrayList<SearchWebResult>	sr2=new ArrayList<SearchWebResult>();
						sr2.add(r);
						conta++;
						resultlist.put(conta, sr2);
					}
				}
				/*************/
	        ...
	        track.setOperation("archivesearchelastic");
	        track.setParam1("");
	        track.setParam2("");
	        track.setParam3("");
		...
		/*for (Integer key : resultlist.keySet()){
			...
			for (SearchWebResult srw : resultlist.get(key)){
				System.out.println(srw.getTitle());
			}
			System.out.println("----");
		}*/
...
private boolean jaccardSimilarity(String content1, String content2) {
		Jaccard js = new Jaccard();
		if(js.similarity(content1,content2) > this.jthreshold)
			return true;
	    return false;
}

private String prettufyTstamp(String tstamp) {
	String res=tstamp.substring(0, 8);
	
	return res.substring(6,8)+"/"+res.substring(4,6)+"/"+res.substring(0,4);
}
